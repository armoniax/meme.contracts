...
void applynewmeme::init(   const name& admin, 
   ...
   _gstate.fufi_apply_contract   = fufi_apply_contract;
...
void applynewmeme::on_transfer(const name& from, const name& to, const asset& quantity, const string& memo){
   ...
   auto paid_amount  = itr->total_supply.quantity.amount/100000000 * itr->init_price;
   CHECKC(paid_amount != quantity.amount, err::PARAM_ERROR, "paid amount invalid");
   CHECKC(from_bank == itr->trade_symbol.get_contract(), err::PARAM_ERROR, "from bank invalid"); 
